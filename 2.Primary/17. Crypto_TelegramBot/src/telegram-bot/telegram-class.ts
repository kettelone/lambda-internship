...
const telegramEndpoint = `https://api.telegram.org/bot${process.env
	.TELEGRAM_TOKEN}`
...
export class UserResponse implements IUserResponse {
	...
	async detailedCoinInfo(coin: string): Promise<string | boolean> {
		const coinMarketResponse = await axios.get<
			CoinMarketResponse
		>(urlCoinmarketcap, {
			headers: { 'X-CMC_PRO_API_KEY': coinMarketKey }
		})
	...
	async listFavourite(chatId: number, userId: number): Promise<void> {
		let cryptoList = ''
		try {
			const favouriteCrypto: Array<ICrypto> = await Crypto.find({
				userId: userId
			})
	...
	async removeFromFavourite(
		...
		try {
			Crypto.deleteOne({ symbol: `${deleteCoin}`, userId: userId }, function(
				err: Error
			) {
				if (err) {
					console.log(err)
